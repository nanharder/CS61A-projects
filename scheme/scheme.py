...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            function = operator.apply_macro(rest, env)
            return scheme_eval(function, env)

        check_args = lambda x: scheme_eval(x, env)
        operands = rest.map(check_args)
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        #if self.bindings.get(symbol): 如果是0会报错
            return self.bindings.get(symbol)
        else:
            parent_frame = self.parent
            while parent_frame != None:
                if parent_frame.bindings.get(symbol):
                    return parent_frame.bindings.get(symbol)
    ...
    def make_child_frame(self, formals, vals):
        ...
        child_frame.bindings = {key:value for key, value in self.bindings.items()}
        while formals != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambdaprocedure = LambdaProcedure(target.second, expressions.second, env)
...
def do_quote_form(expressions, env):
    ...
    if scheme_atomp(expr):
        ...
        return do_quasiquote_form(expr)
    else:
        return expr
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        last_expression = scheme_eval(expressions.first, env)
        if scheme_falsep(last_expression):
            return False
        else:
